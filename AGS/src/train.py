...
def initialization(image_dir, label_dir, roi_dir,
    ...
    transform = torchio.transforms.Compose([torchio.transforms.RandomAffine(scales=(0.9, 1.2), degrees=(10), isotropic=False,default_pad_value='otsu',image_interpolation='BSPLINE'),
    ...
    for epoch in range(current_epoch + 1, total_epoch_num):  # loop over the dataset multiple times
        ...
        if epoch % 1 == 0:
            validation_loss_epoch, validation_dice_avg, accuracy = validation(model, validation_loader,
                                                                    loss_criterion_BCE, loss_criterion_dice, validation_saving_dir, nClass=nClass)
            validation_loss.append(validation_loss_epoch)
            early_stop_condition, save_model_condition, best_dice, stop_index = \
                early_stop(best_dice, validation_dice_avg, stop_index, stop_epoch=int(total_epoch_num/5))
            print('early stop index: {}/{}'.format(stop_index, int(total_epoch_num/5)))
        # save the model
        print(1)
...
def main(image_dir, label_dir, roi_dir,
         ...
         validation_label_dir, validation_roi_dir,
         nClass, prefix=None, classification=False, resume=False, canny=False, vanila=False, total_epoch_number=100):
    ...
    trainer(model, model_saving_dir, loss_criterion_BCE, loss_criterion_dice, optimizer, train_loader, validation_loader, scheduler,
            validation_saving_dir=validation_dir, classification=classification, resume=resume, total_epoch_num=total_epoch_number, nClass=nClass)
...
if __name__ == '__main__':
    ...
    # HD
    image_dir = '/media/hao/easystore/subcortical_segmentation/2021_summer/Hao/AGS_cycleGAN/Dataset/new_train/image'
    label_dir = '/media/hao/easystore/subcortical_segmentation/2021_summer/Hao/AGS_cycleGAN/Dataset/new_train/label'
    roi_dir = label_dir

    validation_image_dir = '/media/hao/easystore/subcortical_segmentation/2021_summer/Hao/AGS_cycleGAN/Dataset/new_train/vali_image'
    validation_label_dir = '/media/hao/easystore/subcortical_segmentation/2021_summer/Hao/AGS_cycleGAN/Dataset/new_train/vali_label'
    ...
    main(image_dir, label_dir, roi_dir,
         validation_image_dir,
         validation_label_dir, validation_roi_dir,
         nClass, prefix=prefix, classification=classification, resume=resume, canny=canny, vanila=vanila, total_epoch_number=total_epoch_number)
